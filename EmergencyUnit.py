...
class EmergencyUnit:
    ...
    def allocate_teams_to_emergency(graph: nx.DiGraph, emergency: Emergency):
        ...
        for unit in EmergencyUnit.response_buildings:
            ...
            node_to_emergency_details[unit]['time'] = 1 if unit.name == emergency.coordinate else nx.algorithms.shortest_path_length(graph, unit.name, emergency.coordinate, weight='adjusted_time', method='dijkstra')
        ...
        for response_unit in sorted_node_to_emergency_details:
            emergency_requirement, available, teams_dispatched = response_unit.check_team_availability(emergency_requirement)
            if available:
        ...
        return winner_nodes, avg_resp, winner_criteria
